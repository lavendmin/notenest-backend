...
- **근거 구분**: (1) 반복 호출 무중복은 위 고정 Clock 테스트로 검증했다. (2) 실측에서는 after 웜 5회 동안 결제
  잡이 매번 processed=34(N=100)/167(N=500)로 대상을 처리했고, 웜 측정 종료 후 `bid.status` 분포와
  `bidder_email_sent`·`auction_failure_email_sent` 플래그가 기대값과 일치했다. (raw에는 사이클별 이메일
  호출 수·smtp4dev 결과가 없어 "사이클별 이메일 0건"을 실측 원본으로 직접 제시하지는 않는다.)
...
- **머신 부하 변동(중요)**: 절대 초는 세션 간 편차가 크다. 같은 after 코드가 매칭 세션에서 웜 36.4s, 이전 덜
  바쁜 세션에서 20.5s로 측정됐다(약 1.8배). 관측된 사실은 대량 LOB 로딩과 높은 힙 점유(3~6GB)가 지속됐다는
  것까지다 — GC 로그·CPU·MariaDB 버퍼·디스크 I/O를 측정하지 않아 **GC·DB 캐시·호스트 부하 등 세션 상태의
  영향을 분리하지 못했다**(GC는 가능한 원인이지 확정 원인이 아니다). 따라서 **절대 초 단독보다 (구조적) 웜 마감
  대상 250→0과 (상대) 약 −40% 단축을 주 지표로 쓴다.**
