...
- **before-N=500 표본·순서**: 웜 **3회** 평균(after는 5회). 또한 before-N=500은 직전 after 실행으로 이미
  정산된 데이터 위에서 측정했다(이메일 플래그 set → 재발송 없음). 정상 상태(steady-state) 웜 사이클의 재처리
  비용이라 after 웜과 같은 성격이지만 "before 먼저, after 나중"의 최상 실험 순서는 아니다. **이력서 대표 줄로
  승격하려면 before를 신규 시드에서 5회부터 다시 측정**한다. before-N=100은 신규 시드에서 첫 사이클(12.7s,
  이메일 포함)과 웜(3.35s)을 분리 관측했다.
