...
> **절대값은 측정 세션에 따라 달라진다.** 같은 after 코드가 이전 별도 측정 세션에서는 웜 20.5s(before 정산데이터 32.3s), 이번 매칭 세션에서는 36.4s로 관측됐다 — 세션 간 편차의 원인은 분리하지 못했다. 세션에 안정적인 지표는 **(구조적) 웜 마감 대상 250→0**과 **(상대) 웜 사이클 약 −36~40% 단축**이며, 절대 초는 "매칭 쌍 실측값(세션 의존)"으로만 쓴다.
...
- 결제 후속 잡의 `findById(Music)` 가 PENDING 곡의 **LOB(오디오 3MB/이미지 300KB)를 매 사이클 로딩**한다.
  이 때문에 N=500 웜 사이클이 36.4s로 **여전히 10초 주기를 초과**한다(이전 별도 세션의 20.5s로도 초과).
...
- **세션 간 편차(중요)**: 절대 초는 세션 간 편차가 크다. 같은 after 코드가 이번 매칭 세션에서 웜 36.4s, 이전
  별도 측정 세션에서 20.5s로 측정됐다(약 1.8배). 관측된 사실은 대량 LOB 로딩과 높은 힙 점유(3~6GB)가 지속됐다는
  것까지다 — GC 로그·CPU·MariaDB 버퍼·디스크 I/O를 측정하지 않아 **세션 간 편차의 원인은 분리하지 못했다**
  (GC·DB 캐시·호스트 부하 등은 가능한 요인이지 확정 원인이 아니다). 따라서 **절대 초 단독보다 (구조적) 웜 마감
